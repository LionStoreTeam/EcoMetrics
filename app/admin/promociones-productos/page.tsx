...
// import {
//     Search, Filter, Eye, CheckCircle, XCircle, Clock, UserCog, Package, Tag, CalendarDays,
//     Loader2, AlertTriangle, Info, BadgeCheck, BadgeX, RotateCcw, ExternalLink
// } from "lucide-react";
...
// import { BusinessPromotionStatus } from "@prisma/client"; // El enum de Prisma
// import { ProductPromotionRequestFE } from "@/types/types"; // Usamos el tipo del frontend
// import { BUSINESS_TYPES } from "@/lib/constants";

// // Interfaz para la respuesta de la API de admin para promociones de productos
// interface AdminProductPromotionsApiResponse {
//     requests: ProductPromotionRequestFE[]; // Asegúrate que este tipo incluya productImageUrls como array de {id, url}
//     pagination: {
//         total: number;
//         page: number;
//         limit: number;
...
//     const [requests, setRequests] = useState<ProductPromotionRequestFE[]>([]);
...
//     const [selectedRequestAdmin, setSelectedRequestAdmin] = useState<ProductPromotionRequestFE | null>(null);
...
//     const openDetailsModalAdmin = (request: ProductPromotionRequestFE) => {
//         setSelectedRequestAdmin(request);
//         setIsDetailsModalOpenAdmin(true);
//     };

//     const openReviewModalAdmin = (request: ProductPromotionRequestFE, targetStatus: BusinessPromotionStatus) => {
//         setSelectedRequestAdmin(request);
...
//             fetchProductPromotionRequests(currentPageAdmin, searchTermAdmin, statusFilterAdmin);
//             router.refresh();
//         } catch (err) {
...
//                 <div className="p-6 flex flex-col gap-2 text-white bg-gradient-to-r from-teal-600 to-cyan-700 rounded-xl shadow-lg">
...
//                                         <TableBody>
//                                             {requests.map((req) => {
//                                                 const StatusIcon = STATUS_MAP_ADMIN[req.status].icon;
//                                                 return (
...
//                             </DialogTitle>
//                             <DialogDescription>
//                                 Solicitado por: {selectedRequestAdmin?.businessName}
//                             </DialogDescription>
//                         </DialogHeader>
...
//                                 {selectedRequestAdmin.productImageUrls && selectedRequestAdmin.productImageUrls.length > 0 && (
//                                     <div className="mb-3 p-2 border-b">
//                                         <Label className="font-semibold text-xs text-gray-500 block mb-1">Imágenes del Producto/Servicio:</Label>
//                                         <div className="flex flex-wrap gap-2 justify-center">
//                                             {selectedRequestAdmin.productImageUrls.map(img => (
//                                                 <Image key={img.id} src={img.url || "/placeholder-business.svg"} alt="Imagen de producto" width={100} height={100} className="rounded-md object-cover border shadow-sm" />
//                                             ))}
//                                         </div>
//                                     </div>
//                                 )}
...
//                                     <p><strong className="font-medium">Tipo:</strong> {getBusinessTypeNameAdmin(selectedRequestAdmin.businessType)}</p>
...
//                                     {selectedRequestAdmin.website && <p><strong className="font-medium">Sitio Web:</strong> <a href={selectedRequestAdmin.website} target="_blank" rel="noopener noreferrer" className="text-teal-600 hover:underline">{selectedRequestAdmin.website}</a></p>}
                            ...
                            <>
                                <div className="overflow-x-auto">
                                    <Table>
                                        ...
                                        <TableBody>
                                            {requests.map((req) => {
                                                const IconForStatus = STATUS_MAP_ADMIN[req.status].icon; // Correcto: Asignar a variable con mayúscula
                                                return (
                                                    <TableRow key={req.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                                        <TableCell>
                                                            <Avatar className="h-9 w-9">
